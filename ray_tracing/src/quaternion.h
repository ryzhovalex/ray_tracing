...
public:
	 ...
	 */
	Quaternion(const Vec3& v, float rotation) {
		rotation /= 2;
		real = cos(rotation);
	 ...
	 */
	Quaternion(const Vec3& v) : real(0), i(v.data[0]), j(v.data[1]), k(v.data[2]) {}
	/**
	...
	Quaternion operator*(const Vec3& right) const;
	Quaternion operator*=(const Vec3& right);
